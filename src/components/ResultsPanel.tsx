...
function formatSpiritLabel(item: SelectedSpirit, data: AppData): string {
  ...
  if (spirit.spiritType === "Aspect") {
    ...
    const baseName = baseSpirit?.name ?? spirit.baseSpiritName ?? "Unknown Base Spirit";
...
function formatBoardLabel(selectedBoard: SelectedBoard): string {
  const isThematic = selectedBoard.boardSide.sideType.toLowerCase().includes("thematic");
  return isThematic ? selectedBoard.boardSide.identifier : selectedBoard.board.name;
}
        ...
        <>
            ...
            {result.selectedSpirits.map((selectedSpirit, index) => {
              ...
              return (
                <li key={selectedSpirit.spirit.canonicalName}>
                  <span className={selectedSpirit.forced ? "forced" : undefined}>
                    {formatSpiritLabel(selectedSpirit, data)}
                  </span>
                  {selectedBoard ? (
                    <>
                      {" on "}
                      <span className={selectedBoard.forced ? "forced" : undefined}>
          ...
          {result.selectedAdditionalBoard && (
            <p className="result-line">
              ...
              <span className={result.selectedAdditionalBoard.forced ? "forced" : undefined}>
          ...
          {result.additionalBoardDroppedWarning && (
            <p className="result-line warning-line">
              ⚠ The additional board was dropped — thematic mode supports a maximum of 6 boards
              total. Disable Thematic Boards or reduce spirits to include it.
            </p>
          )}
          ...
          <div className="launch-links">
            <a href={buildWebLaunchUrl(result, settings)} target="_blank" rel="noreferrer">
              Web launch
            </a>
            <a href={buildSteamLaunchUrl(result, settings)} target="_blank" rel="noreferrer">
              Steam launch
            </a>
          </div>
        </>
